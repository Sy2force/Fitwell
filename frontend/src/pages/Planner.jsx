...
import { Activity, Dumbbell, Apple, ChevronRight, Check, Loader2, Target, Heart, Zap, Clock } from 'lucide-react'
...
const Planner = () => {
  ...
  const handleSubmit = async () => {
    setIsLoading(true)
    try {
      const response = await axios.post('wellness/plans/', formData)
    ...
    <div className="page-container py-20">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl md:text-6xl font-display font-bold text-white mb-6 italic">
            Your Elite <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Blueprint</span>
          </h1>
          <button 
            onClick={onReset}
            className="text-xs text-gray-500 hover:text-accent uppercase tracking-widest border-b border-transparent hover:border-accent transition-all pb-1 font-bold"
          >
            Reset & Recalculate
          </button>
        </div>
        ...
        {/* Analysis Section */}
        {plan.workout_plan.analysis && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-12 glass-card p-8 border-l-4 border-accent"
          >
             <div className="flex flex-col md:flex-row gap-8 items-start">
                {/* Main Score Circle */}
                <div className="flex flex-col items-center justify-center p-6 bg-surface-dark rounded-xl border border-white/10 shrink-0 mx-auto md:mx-0">
                    <span className="text-xs uppercase tracking-widest text-gray-400 mb-4 font-bold">Total Health Score</span>
                    <div className="relative w-40 h-40 flex items-center justify-center">
                        <svg className="w-full h-full transform -rotate-90">
                            ...
                            <circle cx="80" cy="80" r="70" stroke="currentColor" strokeWidth="10" fill="transparent" 
                                className="text-accent drop-shadow-[0_0_10px_rgba(255,0,60,0.5)]" 
                                strokeDasharray={440} 
                                strokeDashoffset={440 - (440 * plan.workout_plan.analysis.score) / 100} 
                                strokeLinecap="round"
                            />
                        </svg>
                        <div className="absolute flex flex-col items-center">
                            <span className="text-5xl font-display font-bold text-white italic">{plan.workout_plan.analysis.score}</span>
                ...
                <div className="flex-1 w-full space-y-6">
                    <div>
                        <h3 className="text-2xl font-display font-bold text-white mb-2 italic">Performance Analysis</h3>
                        <p className="text-gray-400 text-sm font-light leading-relaxed mb-6">
                            &quot;{plan.workout_plan.analysis.message}&quot;
                        </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {plan.workout_plan.analysis.breakdown ? (
                            <>
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <div className="flex items-center text-primary text-xs uppercase tracking-widest font-bold">
                                            <Dumbbell className="w-3 h-3 mr-2" /> Fitness
                                        </div>
                                        <span className="text-white font-mono text-xs">{plan.workout_plan.analysis.breakdown.fitness}%</span>
                                    </div>
                                    <ProgressBar value={plan.workout_plan.analysis.breakdown.fitness} color="primary" showValue={false} />
                                </div>
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <div className="flex items-center text-health text-xs uppercase tracking-widest font-bold">
                                            <Heart className="w-3 h-3 mr-2" /> Recovery
                                        </div>
                                        <span className="text-white font-mono text-xs">{plan.workout_plan.analysis.breakdown.recovery}%</span>
                                    </div>
                                    <ProgressBar value={plan.workout_plan.analysis.breakdown.recovery} color="health" showValue={false} />
                                </div>
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <div className="flex items-center text-energy text-xs uppercase tracking-widest font-bold">
                                            <Zap className="w-3 h-3 mr-2" /> Lifestyle
                                        </div>
                                        <span className="text-white font-mono text-xs">{plan.workout_plan.analysis.breakdown.lifestyle}%</span>
                                    </div>
                                    <ProgressBar value={plan.workout_plan.analysis.breakdown.lifestyle} color="energy" showValue={false} />
                                </div>
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <div className="flex items-center text-gray-400 text-xs uppercase tracking-widest font-bold">
                                            <Clock className="w-3 h-3 mr-2" /> Consistency
                                        </div>
                                        <span className="text-white font-mono text-xs">{plan.workout_plan.analysis.breakdown.consistency}%</span>
                                    </div>
                                    <div className="w-full">
                                        <div className="h-2 bg-white/5 w-full overflow-hidden skew-x-[-10deg]">
                                            <motion.div 
                                                ...
                                                initial={{ width: 0 }}
                                                animate={{ width: `${plan.workout_plan.analysis.breakdown.consistency}%` }}
                            ...
                            <div className="col-span-2 p-4 border border-dashed border-white/10 rounded bg-white/5 text-center text-gray-500 text-xs uppercase tracking-widest">
                                Detailed breakdown available after first check-in
                            </div>
                        )}
                    </div>
                    
                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                        <span className="text-xs uppercase tracking-widest text-gray-500">Body Mass Index</span>
                        <div className="flex items-baseline">
                            <span className="text-2xl font-display font-bold text-white mr-2">{plan.workout_plan.analysis.bmi}</span>
                            <span className="text-[10px] text-gray-500 bg-white/10 px-2 py-1 rounded">KG/M²</span>
                        </div>
                    </div>
                </div>
             </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <motion.div 
            ...
            className="glass-card p-10 border-l-4 border-primary"
          >
            <div className="flex items-center space-x-4 mb-8">
              ...
              <div>
                <h3 className="text-2xl font-display font-bold italic">Training Protocol</h3>
            ...
            <div className="space-y-8">
              <div>
                <span className="block text-[10px] uppercase tracking-[0.2em] text-primary font-bold mb-3">Schedule</span>
                <p className="text-xl text-white font-light">{plan.workout_plan.schedule}</p>
              </div>

              <div>
                <span className="block text-[10px] uppercase tracking-[0.2em] text-primary font-bold mb-3">Core Exercises</span>
                <ul className="space-y-3">
                  {plan.workout_plan.exercises.map((ex, i) => (
                    <li key={i} className="flex items-center text-gray-300 font-light border-b border-white/5 pb-2 group hover:text-white transition-colors">
                      <span className="w-6 h-6 flex items-center justify-center bg-white/5 rounded-full text-xs mr-3 text-primary font-bold group-hover:bg-primary group-hover:text-black transition-all">{i + 1}</span>
                      {ex}
                    </li>
                  ))}
                </ul>
          ...
          <motion.div 
            ...
            className="glass-card p-10 border-l-4 border-secondary"
          >
            <div className="flex items-center space-x-4 mb-8">
              ...
              <div>
                <h3 className="text-2xl font-display font-bold italic">Nutrition Fueling</h3>
                <span className="text-xs text-gray-400 uppercase tracking-widest">Daily Macros</span>
              </div>
            </div>

            <div className="flex flex-col items-center justify-center py-6 mb-8 bg-secondary/5 border border-secondary/10 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-20 h-20 bg-secondary/20 blur-[40px] pointer-events-none"></div>
              <span className="text-5xl font-display font-bold text-white mb-2 italic">{plan.nutrition_plan.calories}</span>
              <span className="text-xs text-secondary uppercase tracking-[0.3em] font-bold">Daily Calories</span>
            </div>
            ...
            <div className="space-y-4">
               <span className="block text-[10px] uppercase tracking-[0.2em] text-secondary font-bold mb-3">Sample Day</span>
               {Object.entries(plan.nutrition_plan.meals).map(([meal, food]) => (
                 <div key={meal} className="flex justify-between border-b border-white/5 pb-2">
                    <span className="text-xs text-gray-400 uppercase tracking-wider w-24 font-bold">{meal}</span>
